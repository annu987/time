...
import { Calendar, Users, Book, Activity, LogOut, Menu, X, Check, Settings, BarChart3, Plus, Trash2, Edit2, RefreshCw, Download, AlertCircle, CheckCircle, Moon, Sun, Bell, Award, TrendingUp, FileText, Clock, MapPin, Zap, Search, Filter, ChevronDown, ChevronRight, Grid, List, AlertTriangle } from 'lucide-react';
...
const AppProvider = ({ children }) => {
  ...
  const generateAITimetable = () => {
    ...
    setTimeout(() => {
      ...
      // Track occupied slots: key = "day-time-room" or "day-time-faculty"
...
const SmartEducationApp = () => {
  const { user, notification, theme } = useAppContext();
...
const Sidebar = ({ sidebarOpen, setSidebarOpen, currentView, setCurrentView }) => {
  const { user, logout, theme, darkMode, toggleTheme } = useAppContext();
  ...
  return (
    <div style={{ background: `linear-gradient(180deg, ${theme.primary} 0%, ${theme.secondary} 100%)` }} className={`${sidebarOpen ? 'w-64' : 'w-20'} text-white transition-all duration-300 flex flex-col shadow-2xl`}>
      ...
      <nav className="flex-1 px-2 py-4 space-y-1 overflow-y-auto">
        ...
        <div className="px-2 py-3 mt-4">{sidebarOpen && <p className="text-xs font-semibold opacity-70 uppercase tracking-wider">Settings</p>}</div>
...
const DashboardView = ({ setCurrentView }) => {
  const { user, timetable, bookings, announcements, theme, generateAITimetable, loading, detectConflicts } = useAppContext();
...
>
  ...
  <p className="text-xs mt-1 text-white opacity-90">PDF & CSV formats</p>
</button>
          </div>
        </div>
      )}
...
const TimetableView = () => {
  ...
  const handleUpdate = (e) => {
    ...
    // Check for conflicts (excluding current editing item)
    const conflict = timetable.find(t => 
      t.id !== editingId &&
      (t.room === formData.room || t.faculty === formData.faculty) &&
      t.day === formData.day &&
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div style={{ backgroundColor: theme.card, borderColor: theme.primary + '20' }} className="rounded-xl shadow-lg p-6 border-2">
            ...
            <table className="w-full">
              <thead style={{ backgroundColor: theme.background }}>
                <tr>
                  ...
                  {user.role === 'admin' && (
                    <th style={{ color: theme.text }} className="px-6 py-4 text-left text-sm font-bold">Actions</th>
                  )}
                </tr>
              </thead>
              <tbody>
...
const SmartClassroomView = () => {
  ...
  const handleSubmit = (e) => {
    ...
    // Check if room is available (not in timetable for that day/time)
    const conflict = timetable.find(t => 
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Classrooms Grid */}
      <div>
        <h4 style={{ color: theme.text }} className="text-lg font-bold mb-4">Available Classrooms</h4>
...
const AnalyticsView = () => {
  ...
  const roomUtilization = Object.entries(roomUsage).map(([room, count]) => ({
    room,
    utilization: ((count / (DAYS.length * TIME_SLOTS.length)) * 100).toFixed(1)
    ...
    <div className="space-y-6">
      ...
      <div style={{ backgroundColor: theme.card }} className="rounded-xl shadow-lg p-6">
        ...
        <div className="space-y-4">
          {roomUtilization.map((stat, idx) => (
            <div key={idx}>
              <div className="flex justify-between items-center mb-2">
                <span style={{ color: theme.text }} className="text-sm font-semibold">{stat.room}</span>
                <span style={{ color: theme.primary }} className="text-sm font-bold">{stat.utilization}%</span>
              </div>
      ...
      <div style={{ backgroundColor: theme.card }} className="rounded-xl shadow-lg p-6">
        ...
        <div className="space-y-4">
          {facultyWorkload.slice(0, 8).map((stat, idx) => (
            <div key={idx}>
              <div className="flex justify-between items-center mb-2">
                <span style={{ color: theme.text }} className="text-sm font-semibold">{stat.faculty}</span>
                <span style={{ color: theme.success }} className="text-sm font-bold">{stat.classes} classes</span>
              </div>
              <div style={{ backgroundColor: theme.background }} className="w-full h-3 rounded-full overflow-hidden">
                <div 
                  style={{ 
                    width: `${(stat.classes / 25) * 100}%`, 
                    background: `linear-gradient(90deg, ${theme.success} 0%, ${theme.warning} 100%)` 
                  }} 
                  className="h-full rounded-full transition-all duration-1000"
                ></div>
              </div>
      ...
      <div style={{ backgroundColor: theme.card }} className="rounded-xl shadow-lg p-6">
        ...
        <div className="space-y-4">
          {dayDistribution.map((stat, idx) => (
            <div key={idx}>
              <div className="flex justify-between items-center mb-2">
                <span style={{ color: theme.text }} className="text-sm font-semibold">{stat.day}</span>
                <span style={{ color: theme.accent }} className="text-sm font-bold">{stat.classes} classes</span>
              </div>
              <div style={{ backgroundColor: theme.background }} className="w-full h-3 rounded-full overflow-hidden">
                <div 
                  style={{ 
                    width: `${(stat.classes / 10) * 100}%`, 
  ...
  return (
    <div className="space-y-6">
      <div style={{ backgroundColor: theme.card }} className="rounded-xl shadow-lg p-6">
        ...
        <div className="space-y-4">
          <div>
            <label style={{ color: theme.text, opacity: 0.7 }} className="block text-sm font-medium mb-2">Full Name</label>
            <input type="text" value={user.name} readOnly style={{ backgroundColor: theme.background, color: theme.text, borderColor: theme.border }} className="w-full px-4 py-3 border rounded-lg" />
          </div>
          <div>
            <label style={{ color: theme.text, opacity: 0.7 }} className="block text-sm font-medium mb-2">Email Address</label>
            <input type="email" value={user.email} readOnly style={{ backgroundColor: theme.background, color: theme.text, borderColor: theme.border }} className="w-full px-4 py-3 border rounded-lg" />
          </div>
          <div>
            <label style={{ color: theme.text, opacity: 0.7 }} className="block text-sm font-medium mb-2">Role</label>
            <input type="text" value={user.role} readOnly style={{ backgroundColor: theme.background, color: theme.text, borderColor: theme.border }} className="w-full px-4 py-3 border rounded-lg capitalize" />
          </div>
          {user.subject && (
            <div>
              <label style={{ color: theme.text, opacity: 0.7 }} className="block text-sm font-medium mb-2">Subject</label>
              <input type="text" value={user.subject} readOnly style={{ backgroundColor: theme.background, color: theme.text, borderColor: theme.border }} className="w-full px-4 py-3 border rounded-lg" />
            </div>
          )}
          {user.rollNo && (
            <div>
              <label style={{ color: theme.text, opacity: 0.7 }} className="block text-sm font-medium mb-2">Roll Number</label>
              <input type="text" value={user.rollNo} readOnly style={{ backgroundColor: theme.background, color: theme.text, borderColor: theme.border }} className="w-full px-4 py-3 border rounded-lg" />
            </div>
          )}
        </div>
      </div>
      ...
      <div style={{ backgroundColor: theme.card }} className="rounded-xl shadow-lg p-6">
        <h4 style={{ color: theme.text }} className="text-lg font-bold mb-4">Notification Preferences</h4>
        <div className="space-y-3">
          <label className="flex items-center gap-3 cursor-pointer">
            <input type="checkbox" defaultChecked className="w-5 h-5 rounded" style={{ accentColor: theme.primary }} />
            <span style={{ color: theme.text }}>Email notifications for schedule changes</span>
          </label>
          <label className="flex items-center gap-3 cursor-pointer">
            <input type="checkbox" defaultChecked className="w-5 h-5 rounded" style={{ accentColor: theme.primary }} />
            <span style={{ color: theme.text }}>Push notifications for upcoming classes</span>
          </label>
          <label className="flex items-center gap-3 cursor-pointer">
            <input type="checkbox" className="w-5 h-5 rounded" style={{ accentColor: theme.primary }} />
            <span style={{ color: theme.text }}>Weekly schedule summary</span>
          </label>
        </div>
      </div>

      <div style={{ backgroundColor: theme.card }} className="rounded-xl shadow-lg p-6">
        <h4 style={{ color: theme.text }} className="text-lg font-bold mb-4">System Information</h4>
        <div className="space-y-2 text-sm" style={{ color: theme.text, opacity: 0.8 }}>
          <p><strong>System:</strong> Smart Education v2.0</p>
          <p><strong>Build:</strong> Hackathon Edition</p>
          <p><strong>Features:</strong> AI Scheduling, Smart Classroom, Real-time Analytics</p>
